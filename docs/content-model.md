...
| **item**       | Open5e raw      | `kind` (wondrous/weapon/armor/gear/consumable), `rarity`, `requires_attunement` (+ `attunement_detail`), `weight`, `cost`, `charges`, plus `weapon` or `armor` stats when the kind calls for them, and a `use` block (action cost, effect, dice, cure, consumed) for a thing that is used rather than carried |
